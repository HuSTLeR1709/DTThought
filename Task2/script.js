document.addEventListener('DOMContentLoaded', function() {
    ...
    const videoUrl = dataset.tasks[0].assets[0].asset_content;

    // Set the src attribute of the iframe
    const videoFrame = document.getElementById('videoFrame');
    videoFrame.src = videoUrl;

    function setCardContent(assetTitle, assetDescription, targetElementId) {
        // Set asset title
        document.getElementById(targetElementId + 'head').textContent = assetTitle;
    
        // Set asset description
        document.getElementById(targetElementId + 'desc').textContent = 'Description : ' + assetDescription;
    }
    
    // Usage of the function for each card
    const assetTitle1 = dataset.tasks[0].assets[0].asset_title;
    const assetDesc1 = dataset.tasks[0].assets[0].asset_description;
    setCardContent(assetTitle1, assetDesc1, 'card1');
    
    const assetTitle2 = dataset.tasks[0].assets[1].asset_title;
    const assetDesc2 = dataset.tasks[0].assets[1].asset_description;
    setCardContent(assetTitle2, assetDesc2, 'card2');
    
    const assetTitle3 = dataset.tasks[0].assets[2].asset_title;
    const assetDesc3 = dataset.tasks[0].assets[2].asset_description;
    setCardContent(assetTitle3, assetDesc3, 'card3');
    
    const assetTitle4 = dataset.tasks[0].assets[3].asset_title;
    const assetDesc4 = dataset.tasks[0].assets[3].asset_description;
    setCardContent(assetTitle4, assetDesc4, 'card4');
